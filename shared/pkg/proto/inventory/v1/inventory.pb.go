...
// Category определяет тип детали.
type Category int32
...
// Part представляет собой отдельный компонент для сборки космических кораблей.
type Part struct {
...
// Dimensions описывает физический размер и вес детали.
type Dimensions struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Length        float64                `protobuf:"fixed64,1,opt,name=length,proto3" json:"length,omitempty"`
	Widht         float64                `protobuf:"fixed64,2,opt,name=widht,proto3" json:"widht,omitempty"`
...
func (x *Dimensions) GetWidht() float64 {
	if x != nil {
		return x.Widht
	}
	return 0
...
// Manufacturer содержит информацию о названии, стране и веб-сайте производителя.
...
// Value - это гибкий тип для хранения различных видов метаданных.
type Value struct {
	state protoimpl.MessageState `protogen:"open.v1"`
...
// GetPartRequest - сообщение запроса для метода GetPart.
...
// GetPartResponse - сообщение ответа для метода GetPart.
...
// ListPartsRequest - сообщение запроса для метода ListParts.
...
// ListPartsResponse - сообщение ответа для метода ListParts.
...
const file_inventory_v1_inventory_proto_rawDesc = "" +
	...
	"\x05widht\x18\x02 \x01(\x01R\x05widht\x12\x16\n" +
	...
	"\tListParts\x12\x1e.inventory.v1.ListPartsRequest\x1a\x1f.inventory.v1.ListPartsResponseBUZSgithub.com/shenikar/microservices-course/shared/pkg/proto/inventory/v1;inventory_v1b\x06proto3"
...
var file_inventory_v1_inventory_proto_depIdxs = []int32{
	...
	1,  // 8: inventory.v1.ListPartsResponse.parts:type_name -> inventory.v1.Part
